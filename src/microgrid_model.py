...
def summarize_year(result: YearResult, start_index: int = 31) -> dict:
    ...
    return {
        ...
        "soc_min_kwh": float(min(np.min(result.soc_start[sl]), np.min(result.soc_end[sl]))),
        "soc_max_kwh": float(max(np.max(result.soc_start[sl]), np.max(result.soc_end[sl]))),
...
def validate_result(data: DataBundle, result: YearResult, price: np.ndarray, name: str) -> dict:
    ...
    checks = {
        ...
        "soc_bounds": bool(
            np.min(np.r_[result.soc_start, result.soc_end]) >= SOC_MIN - 1.0e-6
            and np.max(np.r_[result.soc_start, result.soc_end]) <= SOC_MAX + 1.0e-6
        ),
...
def run(root: Path) -> dict:
    ...
    q2 = simulate_fixed_policy(data, forecasts["q2_robust"], data.fixed_price)
    ...
    summary = {
        ...
        "problem2_model_selection": {
            "method": "仅用1月数据进行时间外参数校准，2—12月固定使用所选分位数",
            "objective": "计划购电成本+缺电惩罚成本最小",
            "selected_quantile": q2_quantile,
            "candidates": q2_tuning,
        },
        "problem1": q1,
